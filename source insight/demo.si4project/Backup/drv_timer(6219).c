...
void drv_timer1_init(void)
{
	...
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF;//GPIO参数配置
	...
	GPIO_PinAFConfig(GPIOE, GPIO_PinSource9, GPIO_AF_TIM1);//指定复用功能为TIM1
	...
	TIM_OCInitStruct.TIM_OCIdleState = TIM_OCIdleState_Set;//空闲时为高
	...
	TIM_OC1PreloadConfig(TIM1, TIM_OCPreload_Enable);//使能CCR1的装载器
