...
TaskHandle t start_task_handle;

void drv_start_stack(void){
	while (1)
	{
		printf("")	
	}
...
//	printf("W25QXX_readID=0x%x\r\n",drv_w25qxx_readID());
	...
	//drv_dma_init(dma_src, dma_drc, 10);
	//while(DMA_GetFlagStatus(DMA2_Stream0, DMA_FLAG_TCIF0) == RESET);
	//drv_dma_datacompare();

	//drv_adc_init();
	xTaskCreate(drv_start_stack,"drv_start_stack" , 128,NULL, 1 ,&start_task_handle);
	
	while(1)
	{
//		printf("drv_adcdata_sum = %d\r\n",drv_adcdata_sum());
//		drv_systick_delayms(1000);
	}
}
